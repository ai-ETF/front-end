<template>
  <div class="page-container">
    ...
    <div class="input-container">
      <div class="input-group">
        <div class="input-frame">
          ...
          <input v-model="username" type="text" placeholder="用户名（可选）" class="input-field" />
        </div>
      </div>
      ...
      <!-- 注册按钮 -->
      <button @click="handleRegister" :disabled="loading" class="continue-btn">
        {{ loading ? '注册中...' : '创建账户' }}
      </button>
    </div>
    ...
    <div class="bottom-text">
      已有账户？ 
      <router-link to="/login" class="link-register">登录</router-link>
    </div>
  </div>
</template>
...
const handleRegister = async () => {
  try {
    ...
    const { user, error } = await register(email.value, password.value, username.value)
