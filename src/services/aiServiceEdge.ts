...
//   // Edge Function URL
//   const EDGE_FUNCTION_URL = 'https://wiynpkkfsiiqnofhifhs.supabase.co/functions/v1/ask-ai';

//   console.log('🔵 开始流式请求', {
//     message: lastUserMessage.text.substring(0, 50) + (lastUserMessage.text.length > 50 ? '...' : ''),
...
//         console.log(`📄 原始数据 (${chunkText.length}字符):`,
...
//           console.log(`📋 处理行: ${trimmedLine.substring(0, 80)}${trimmedLine.length > 80 ? '...' : ''}`);
...
//             } catch (parseError) {
//               console.warn('⚠️  解析JSON失败:', parseError, '原始数据:', dataContent.substring(0, 100));

//               // 如果不是JSON，可能是纯文本，直接作为内容处理
//               if (dataContent && dataContent !== '[DONE]') {
//                 console.log(`✅ 将非JSON数据作为内容: "${dataContent.substring(0, 50)}..."`);
...
//               // 同样的内容提取逻辑
//               const content = data.choices?.[0]?.delta?.content || data.content || data.message;
//               if (content && typeof content === 'string') {
//                 console.log(`✅ 提取到内容: "${content}"`);
//                 fullResponse += content;
//                 onChunk(content);
...
//       return {
//         success: false,
//         error: `流处理异常: ${streamError instanceof Error ? streamError.message : '未知错误'}`
//       };
//     } finally {
//       reader.releaseLock();
//       console.log('🔒 已释放读取器锁');
...
//       return { success: false, error: '网络连接失败，请检查网络设置和URL是否正确' };
//     }
